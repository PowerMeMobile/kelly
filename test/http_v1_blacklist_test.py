...
def http(request):
    ...
    def fin():
        print ("finalizing...")
        http.delete(BLACKLIST_URL+'/'+ENTRY_ID)

    request.addfinalizer(fin)
    return http

def test_create_entry_succ(http):
    req = http.post(BLACKLIST_URL, data={'id':ENTRY_ID,
...
def test_read_succ(http):
    ...
    req = http.get(BLACKLIST_URL+'/'+ENTRY_ID)
...
def test_update_entry_1_succ(http):
    ...
    req = http.put(BLACKLIST_URL+'/'+ENTRY_ID, data={'dst_addr':'375296000003,1,1',
                                                      'src_addr':''})
    assert req.status_code == 200
    json = req.json()
    assert json['id'] == ENTRY_ID
...
def test_update_entry_2_succ(http):
    ...
    req = http.put(BLACKLIST_URL+'/'+ ENTRY_ID, data={'dst_addr':'375296000004,1,1'})
    assert req.status_code == 200
    json = req.json()
    assert json['id'] == ENTRY_ID
...
def test_update_entry_3_succ(http):
    ...
    req = http.put(BLACKLIST_URL+'/'+ ENTRY_ID, data={'src_addr':'Hola,5,0'})
    assert req.status_code == 200
    json = req.json()
    assert json['id'] == ENTRY_ID
    assert json['dst_addr']['addr'] == '375296000001'
...
def test_update_entry_4_succ(http):
    ...
    req = http.put(BLACKLIST_URL+'/'+ ENTRY_ID, data={})
    assert req.status_code == 200
    json = req.json()
    assert json['id'] == ENTRY_ID
    assert json['dst_addr']['addr'] == '375296000001'
...
def test_delete_succ(http):
    ...
    req = http.delete(BLACKLIST_URL+'/'+ENTRY_ID)
