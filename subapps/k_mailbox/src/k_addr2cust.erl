...
resolve(Address = #addr{}) ->
    ...
    case mongodb_storage:find(static_storage, mb_msisdns, Selector) of
...
link(Address = #addr{}, CustID, UserID) ->
    ...
    case resolve(Address) of
        {error, addr_not_used} ->
            {ok, _ID} = mongodb_storage:insert(static_storage, mb_msisdns, Modifier),
            ok;
        _ -> {error, addr_in_use}
    end.
...
unlink(Address = #addr{}) ->
    ...
    ok = mongodb_storage:delete(static_storage, mb_msisdns, Selector).
...
available_addresses(CustID, UserID) ->
    ...
    {ok, Docs} = mongodb_storage:find(static_storage, mb_msisdns, Selector),
