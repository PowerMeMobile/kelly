...
set_mt_req_info(#req_info{
	...
	case mongodb_storage:command(k_previous_dynamic_storage, Command) of
		{ok, {value, _, lastErrorObject, {updatedExisting, true, n, 1}, ok, _}} ->
			ok;
		{ok, {value, undefined, ok, _}} ->
			mongodb_storage:upsert(k_current_dynamic_storage, mt_messages, Selector, Plist)
	end.
...
set_mt_dlr_info(#dlr_info{
	...
	mongodb_storage:upsert(k_previous_dynamic_storage, mt_messages, Selector, Plist).

-spec get_mt_msg_info(gateway_id(), msg_id()) -> {ok, #msg_info{}} | {error, reason()}.
get_mt_msg_info(GatewayId, OutMsgId) ->
	Selector = [{gi, GatewayId}, {omi, OutMsgId}],
	case mongodb_storage:find_one(k_previous_dynamic_storage, mt_messages, Selector) of
		{ok, Plist} ->
...
set_mo_msg_info(MsgInfo = #msg_info{}) ->
	...
	mongodb_storage:upsert(k_previous_dynamic_storage, mo_messages, Selector, Plist).

-spec get_mo_msg_info(binary(), any()) -> {ok, #msg_info{}} | {error, reason()}.
get_mo_msg_info(GatewayId, InMsgId) ->
	Selector = [{gi, GatewayId}, {imi, InMsgId}],
	case mongodb_storage:find_one(k_previous_dynamic_storage, mo_messages, Selector) of
		{ok, Plist} ->
