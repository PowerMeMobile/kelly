...
set_provider(ProviderId, Provider)->
	...
	mongodb_storage:upsert(providers, [{'_id', ProviderId}], Plist).

-spec get_provider(provider_id()) -> {ok, #provider{}} | {error, no_entry} | {error, term()}.
get_provider(ProviderId) ->
	case mongodb_storage:find_one(providers, [{'_id', ProviderId}]) of
...
get_providers() ->
	case mongodb_storage:find(providers, []) of
		{ok, List} ->
			{ok, [
...
del_provider(ProviderId) ->
	mongodb_storage:delete(providers, [{'_id', ProviderId}]).
