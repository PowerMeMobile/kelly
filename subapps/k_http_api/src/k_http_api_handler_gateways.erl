...
delete(Params) ->
    ...
    case k_storage_gateways:get_gateway(Uuid) of
        {ok, #gateway{connections = Cs, settings = Ss}} ->
            ...
            [ok = k_support_just:delete_connection(Uuid, C#connection.id) || C <- Cs],
            [ok = k_support_just:delete_setting(Uuid, S#setting.name) || S <- Ss],
            ok = k_support_just:delete_gateway(Uuid),
...
update_gtw(Gtw, Params) ->
    ...
    ok = k_support_just:set_gateway(Uuid, NewName, NewRPS),
...
create_gtw(Params) ->
    ...
    ok = k_support_just:set_gateway(Uuid, Name, RPS),
